...
rsync -avC -e "ssh -i ../ec2.pem" /home/ugik/Blitz/ ubuntu@"$1":/home/ubuntu/Blitz --exclude 'usermedia' --exclude 'migrations' --exclude 'collected_static' --exclude 'bin/*' --exclude 'lib/*' --exclude '.Python' --exclude 'local_settings.*' --exclude 'database.*' --exclude '.git' --exclude '*.conf' --exclude '.sass-cache' --exclude '*.*~' --exclude 'logs' --delete
